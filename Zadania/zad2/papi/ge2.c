...
// gcc -mavx $(pkg-config --cflags --libs papi) -lm ge2.c -o ge2.o
// ./ge2.o 1000
...
int main(int argc, const char *argv[])
{
  ...
  // Size, time, checksum, total cycles, total instructions executed, L1 data cache misses, L2 instruction cache hits
  printf("%d,%le,%lld,%lld,%lld,%lld,%lf\n", SIZE, dtime, values[0], values[1], values[2], values[3], check);
  fflush(stdout);
